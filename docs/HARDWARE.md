...
- Two 128×128×8bpp framebuffers (16 KB each, in one dual-port 32K×8 SRAM).
  ...
  flipped via VID_OUT_PAGE). **Core deviation (M8–M9.3):** the emulator
  presents one immutable framebuffer image per host frame, so the core copies
  the page requested by VID_OUT_PAGE into a dedicated 16 KB display snapshot
  during vertical blank. Active scanout reads only that snapshot. This hides
  both sub-frame `$2007` transients (Ganymede clears bit 1 for ~4k CPU cycles
  during engine housekeeping, `test_vidpage_latch`) and writes made after a
  game releases and immediately recycles a frontbuffer (Minicraft,
  `test_frontbuffer_ownership`). The 16,386-clock copy fits inside the
  29,056-clock vertical blank.

...
int main(int argc, char** argv) {
    ...
    FILE* sf = std::getenv("GT_READ_STREAM")
                   ? std::fopen(std::getenv("GT_READ_STREAM"), "wb")
                   : nullptr;
    ...
    while (flips < 810 && sim.cycles < guard) {
        ...
        if (census && r->gametank__DOT__cpu_ce) {
            ...
            if (sf && (sel == SEL_PAD || sel == SEL_VIA || sel == SEL_ARAM ||
                       sel == SEL_OPEN)) {
                uint8_t v = 0;
                if (sel == SEL_ARAM) v = sim.top.rootp->gametank__DOT__acp__DOT__aram[wa & 0xFFF];
                else if (sel == SEL_OPEN) v = sim.top.rootp->gametank__DOT__mainbus__DOT__open_bus;
                uint8_t rec[4] = {sel, (uint8_t)(wa & 0xFF), (uint8_t)(wa >> 8), v};
                std::fwrite(rec, 1, 4, sf);
            }
            prevSel = sel;
        }
    ...
    (void)prevSel;
    if (sf) std::fclose(sf);
    if (const char* rp = std::getenv("GT_RAM_AT_READY")) {
        FILE* rf = std::fopen(rp, "wb");
        for (int i = 0; i < 0x8000; i++) std::fputc(sim.sysram((uint16_t)i), rf);
        std::fclose(rf);
    }
